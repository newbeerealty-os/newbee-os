...
export interface ThemeTokens {
  ...
  /** 圆角（px） */
  radius: number;
  /** 图表色：status = 已收 → 已成交 → 签约中 → 预计（同一色相由深到浅）；sides = 卖方 / 买方 / 放租 / 寻租 / 推荐费（固定顺序）；ded = 扣除项的灰 */
  viz: VizTokens;
}
export interface VizTokens { status: [string, string, string, string]; sides: [string, string, string, string, string]; ded: [string, string, string] }
...
/** a 与 b 按 t（0..1）混合，t=0 全是 a */
export const mixHex = (a: string, b: string, t: number) => { const [r1, g1, b1] = rgb(a); const [r2, g2, b2] = rgb(b); return `#${hex(r1 + (r2 - r1) * t)}${hex(g1 + (g2 - g1) * t)}${hex(b1 + (b2 - b1) * t)}`; };
const DAY_SIDES: VizTokens['sides'] = ['#2a78d6', '#eb6834', '#1baf7a', '#eda100', '#e87ba4'];
const NIGHT_SIDES: VizTokens['sides'] = ['#3987e5', '#d95926', '#199e70', '#c98500', '#d55181'];
/** 按主题的 accent / surface / muted 推出一套预设；每个主题的值在下面表里写死（可单独调） */
export const vizFor = (mode: ThemeMode, accent: string, accentStrong: string, surface: string, muted: string): VizTokens => ({
  status: [accentStrong, accent, mixHex(accent, surface, mode === 'day' ? 0.45 : 0.4), mixHex(accent, surface, mode === 'day' ? 0.78 : 0.72)],
  sides: mode === 'day' ? DAY_SIDES : NIGHT_SIDES,
  ded: [muted, mixHex(muted, surface, 0.35), mixHex(muted, surface, 0.65)],
});
...
export const THEMES: Theme[] = [
  ...
  { id: 'blueprint', mode: 'day', swatch: ['#14315c', '#eef2f7', '#1f8fd6', '#ffffff', '#1e7f4f'], tokens: {
    ...
    viz: { status: ['#0f6db0', '#1f8fd6', '#84c1e8', '#cee6f6'], sides: ['#2a78d6', '#eb6834', '#1baf7a', '#eda100', '#e87ba4'], ded: ['#5b6b85', '#949fb0', '#c6cbd4'] } } },
  { id: 'porcelain', mode: 'day', swatch: ['#fbfaf7', '#f3f1eb', '#1f5a46', '#1a1a1a', '#8a5a00'], tokens: {
    ...
    viz: { status: ['#174535', '#1f5a46', '#84a499', '#cedbd6'], sides: ['#2a78d6', '#eb6834', '#1baf7a', '#eda100', '#e87ba4'], ded: ['#6f6a60', '#a19e98', '#cdcbc7'] } } },
  { id: 'sandstone', mode: 'day', swatch: ['#f3efe8', '#2b2f36', '#4a6fa5', '#fffdf9', '#2e7d5b'], tokens: {
    ...
    viz: { status: ['#365688', '#4a6fa5', '#9bafcb', '#d7dee7'], sides: ['#2a78d6', '#eb6834', '#1baf7a', '#eda100', '#e87ba4'], ded: ['#6c7078', '#9fa1a5', '#cccccc'] } } },
  { id: 'steel', mode: 'day', swatch: ['#ffffff', '#f4f6f8', '#0e7c86', '#1f2933', '#1f7a4d'], tokens: {
    ...
    viz: { status: ['#0a5f67', '#0e7c86', '#7ab7bc', '#cae2e4'], sides: ['#2a78d6', '#eb6834', '#1baf7a', '#eda100', '#e87ba4'], ded: ['#62727f', '#99a3ac', '#c8ced2'] } } },
  { id: 'ivory', mode: 'day', swatch: ['#faf8f2', '#23262b', '#a88a3d', '#ffffff', '#2f6b4f'], tokens: {
    ...
    viz: { status: ['#7d6527', '#a88a3d', '#cfbf94', '#ece5d4'], sides: ['#2a78d6', '#eb6834', '#1baf7a', '#eda100', '#e87ba4'], ded: ['#766f60', '#a6a198', '#cfcdc7'] } } },
  // ---------- 夜间 ----------
  { id: 'obsidian', mode: 'night', swatch: ['#0f1420', '#161d2b', '#c9a35b', '#e6e9ef', '#6cc38a'], tokens: {
    ...
    viz: { status: ['#e2c079', '#c9a35b', '#816d48', '#484338'], sides: ['#3987e5', '#d95926', '#199e70', '#c98500', '#d55181'], ded: ['#8b95a8', '#626b7c', '#3f4757'] } } },
  { id: 'graphite', mode: 'night', swatch: ['#1c1c1e', '#26262a', '#d08a5b', '#f2efe9', '#7fcb9a'], tokens: {
    ...
    viz: { status: ['#e8a677', '#d08a5b', '#8c6247', '#564238'], sides: ['#3987e5', '#d95926', '#199e70', '#c98500', '#d55181'], ded: ['#9c9a94', '#73716f', '#4f4f4f'] } } },
  { id: 'jade', mode: 'night', swatch: ['#0b1512', '#12211c', '#3fbf9a', '#e9c56a', '#e3f1ea'], tokens: {
    ...
    viz: { status: ['#6fdcbb', '#3fbf9a', '#2d8068', '#1f4d3f'], sides: ['#3987e5', '#d95926', '#199e70', '#c98500', '#d55181'], ded: ['#7fa392', '#597669', '#384f45'] } } },
  { id: 'indigo', mode: 'night', swatch: ['#12122b', '#1a1a3d', '#7c6cff', '#4fd1c5', '#e9e9ff'], tokens: {
    ...
    viz: { status: ['#a89dff', '#7c6cff', '#554bb1', '#353173'], sides: ['#3987e5', '#d95926', '#199e70', '#c98500', '#d55181'], ded: ['#9a9ccb', '#6d6f99', '#47486f'] } } },
  { id: 'deepsea', mode: 'night', swatch: ['#071a2b', '#0d2740', '#ffb347', '#e6f0f8', '#5ad4a0'], tokens: {
    ...
    viz: { status: ['#ffc978', '#ffb347', '#9e7b44', '#514e42'], sides: ['#3987e5', '#d95926', '#199e70', '#c98500', '#d55181'], ded: ['#86a3bd', '#5c7891', '#37526c'] } } },
];
...
export function themeCss(): string {
  return THEMES.map((t) => {
    const vars = Object.entries(t.tokens).filter(([k]) => k !== 'viz').map(([k, v]) => `--${kebab(k)}:${typeof v === 'number' ? `${v}px` : v}`).join(';');
    const viz = [...t.tokens.viz.status.map((c, i) => `--viz-status-${i + 1}:${c}`), ...t.tokens.viz.sides.map((c, i) => `--viz-side-${i + 1}:${c}`), ...t.tokens.viz.ded.map((c, i) => `--viz-ded-${i + 1}:${c}`)].join(';');
    return `[data-theme="${t.id}"]{color-scheme:${t.mode === 'night' ? 'dark' : 'light'};${vars};${viz}}`;
  }).join('\n');
}
