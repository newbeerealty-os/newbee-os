...
export default async function LanguageSettingsPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-4">
      ...
      <SearchBox placeholder={t("settings.filter")} label={t("common.search")} allLabel={t("common.searchAll")} items={Object.keys(MESSAGES).map((k) => ({ label: k, text: `${k} ${MESSAGES[k].zh} ${MESSAGES[k].en}` }))} widthClass="w-80" />
