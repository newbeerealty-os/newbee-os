...
import { recurringPerPeriod } from "@newbee/core";
...
export default async function CommissionPlanPage() {
  ...
  const keys = ["presets", "presetApplied", "split", "splitPreCap", "splitPostCap", "cap", "capAmount", "capHint", "capYearStart", "perDeal", "perDealFee", "perDealFeeLease", "perDealFeePostCap", "eoFee", "royalty", "royaltyPct", "royaltyCap", "team", "teamPct", "teamCap", "teamBasis", "recurring", "recurringAdd", "recurringName", "recurringAmount", "recurringPeriod", "recurringTotal"] as const;
  const l: Record<string, string> = Object.fromEntries(keys.map((k) => [k, t(`plan.${k}`)]));
  for (const id of ["perDeal", "annual", "capSplit", "splitOnly"]) { l[`preset_${id}`] = t(`plan.preset.${id}`); l[`preset_${id}_desc`] = t(`plan.preset.${id}.desc`); }
