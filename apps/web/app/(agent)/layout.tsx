...
export default async function AgentLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-dvh flex-col md:flex-row">
      {/* 桌面：左栏；手机：底部 tab */}
      <nav className="fixed inset-x-0 bottom-0 z-10 flex items-center justify-around border-t border-zinc-200 bg-white py-2 pb-[env(safe-area-inset-bottom)] md:static md:w-52 md:flex-col md:items-stretch md:justify-start md:gap-1 md:border-r md:border-t-0 md:p-4">
        ...
        <div className="flex items-center gap-2 md:mt-auto md:flex-col md:items-start md:gap-1 md:px-3 md:pt-4">
          ...
          <form action="/auth/signout" method="post" className="hidden md:block">
            <button className="py-2 text-xs text-zinc-400">{t("nav.signout")}</button>
          </form>
        </div>
      </nav>
