...
export function TaskItem({ t, today, backTo, showDeal = false }: { t: TaskRow; today: string; backTo: string; showDeal?: boolean }) {
  const done = !!t.done_at;
  return (
    <li className="flex items-start gap-3 py-2">
      <form action={toggleTask.bind(null, t.id, !done, backTo)}>
        <button aria-label={done ? "标记未完成" : "标记完成"} className={`mt-0.5 flex h-5 w-5 items-center justify-center rounded border ${done ? "border-emerald-500 bg-emerald-500 text-white" : "border-zinc-300 bg-white"}`}>
          {done ? "✓" : ""}
        </button>
      </form>
      <div className="min-w-0 flex-1">
        <div className={`text-sm ${done ? "text-zinc-400 line-through" : "text-zinc-800"}`}>{t.title}</div>
        <div className="mt-0.5 flex flex-wrap items-center gap-2 text-xs text-zinc-500">
          {showDeal && t.deal_id && (
            <Link href={`/deals/${t.deal_id}`} className="text-[#1f5f8b] hover:underline">{t.deal_title ?? "交易"}</Link>
          )}
          {t.due_date && !done && <Badge tone={dueTone(t.due_date, today)}>{t.due_date} · {relDays(t.due_date, today)}</Badge>}
        </div>
      </div>
    </li>
  );
