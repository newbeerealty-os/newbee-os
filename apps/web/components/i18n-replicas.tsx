  ...
  plan: (T, h) => (
    <div className="flex flex-col gap-3">
      ...
      <Card title={T("plan.presets")} right={T("plan.presetApplied")}>
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{["perDeal", "annual", "capSplit", "splitOnly"].map((id, i) => <div key={id} className={`rounded-md border px-2 py-1.5 ${i === 0 ? "border-accent bg-accent-soft" : "border-line-strong"}`}><div className="text-xs font-medium">{T(`plan.preset.${id}`)}</div><div className="text-[11px] text-muted">{T(`plan.preset.${id}.desc`)}</div></div>)}</div>
      </Card>
      <Card title={T("plan.perDeal")} right={<Chip tone="info">{T("plan.module.on")}</Chip>}><div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">{[["plan.perDealFee", "$540"], ["plan.perDealFeeLease", "$125"], ["plan.perDealFeePostCap", "$0"], ["plan.eoFee", "$0"]].map(([k, v]) => <label key={k} className="flex flex-col gap-1"><span className="text-muted">{T(k)}</span><Input>{v}</Input></label>)}</div></Card>
      ...
      <Card title={T("plan.split")} right={<Chip>{T("plan.module.off")}</Chip>}><div className="grid grid-cols-2 gap-2 text-xs">{[["plan.splitPreCap", "70 %"], ["plan.splitPostCap", "100 %"]].map(([k, v]) => <label key={k} className="flex flex-col gap-1"><span className="text-muted">{T(k)}</span><Input>{v}</Input></label>)}</div></Card>
