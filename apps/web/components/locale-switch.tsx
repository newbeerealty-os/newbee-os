...
export async function LocaleSwitch({ className = "" }: { className?: string }) {
  ...
  return (
    <form action={setLocale} className={`inline-flex overflow-hidden rounded-md border border-line-strong text-xs ${className}`}>
      {LOCALES.map((l) => (
        <button key={l} name="locale" value={l} disabled={l === locale} aria-current={l === locale ? "true" : undefined}
          className={`px-2 py-1 ${l === locale ? "bg-accent text-accent-ink" : "bg-surface text-muted hover:bg-chip"}`}>
          {t(`locale.${l}`)}
        </button>
      ))}
    </form>
